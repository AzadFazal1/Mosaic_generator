               ...
               int outpipe_fd = -1)
{
	...
	if (pid == 0)
	{
		...
		char path[] = "PATH=/bin/:/usr/bin:/usr/local/bin:/class/cs225/llvm/bin";
		...
		// Keep around environment variables for gnome-keyring to enable
		// SVN to remember passwords without resorting to always plain-text
		std::string gkr_socket;
		if (const char* gkr_env = ::getenv("GNOME_KEYRING_SOCKET"))
		{
			gkr_socket = "GNOME_KEYRING_SOCKET=";
			gkr_socket.append(gkr_env);
		}

		std::string gkr_pid;
		if (const char* gkr_env = ::getenv("GNOME_KEYRING_PID"))
		{
			gkr_pid = "GNOME_KEYRING_PID=";
			gkr_pid.append(gkr_env);
		}

		std::string dbus_session;
		if (const char* dbus_env = ::getenv("DBUS_SESSION_BUS_ADDRESS"))
		{
			dbus_session = "DBUS_SESSION_BUS_ADDRESS=";
			dbus_session.append(dbus_env);
		}

		char * newenv[] = { path, libpath, licenses, redirect_glibc,
		                    asan_options,
		                    gkr_socket.empty()   ? NULL : const_cast<char*>(gkr_socket.c_str()),
		                    gkr_pid.empty()      ? NULL : const_cast<char*>(gkr_pid.c_str()),
		                    dbus_session.empty() ? NULL : const_cast<char*>(dbus_session.c_str()),
		                    NULL };
		environ = newenv;
