...
class kd_tree
{
  ...
  private:
    ...
    */
    ...
    int euclideanDistance(const point<Dim> &first, const point<Dim> &second)const;

    void build_kd_tree(std::vector< point<Dim> > & newpoints, int left, int right, int curDim);

    point<Dim> select(std::vector< point<Dim> > & list, int left, int right, int k, int curDim);

    int partition(std::vector< point<Dim> > & list, int left, int right, int pivotIndex, int curDim);
